...
In image processing, it seems that flat regions extend when an image get smooth. This would be a wrong assumption in image processing if the first law of the mentioned two restrictions has been hold which means the threshold for computing <i>relative*</i> strong and <i>relative*</i> flat regions holds constant. In this case, quite the contrary, flat regions are contracted and that is the <b>relative**</b> flat regions, which belong to <i>relative*</i> strong regions, are expanded in smoothed images. In other words, in smoothed images, the edges propagate and replace flat regions unlike the flat regions which are repressed. You can follow this phenomenon in the below images.
